...
export const DailsQuizScreen = ({ navigation }: any) => {
  ...
  const handleComplete = async () => {
    try {
      const isSuccess = await submitQuiz();

      console.log(isSuccess, "<<<<QUIZ STATUS");

      // Only show success animation if submission succeeded
      if (isSuccess) {
        setShowSuccess(true);
...
const handleSuccessComplete = async () => {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      {showSuccess && (
        <>
          {console.log('🎉 Rendering SuccessOverlay')}
